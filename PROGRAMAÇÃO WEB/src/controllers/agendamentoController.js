...
class AgendamentoController {
        ...
        try {
            ...
            agendamentos = agendamentos.map(a => {
                ...
                if (a.data) {
                    ...
                    if (partes.length === 3) {
                        a.dataFormatada = `${partes[2]}/${partes[1]}/${partes[0]}`;
                    } else {
                        a.dataFormatada = a.data;
                    }
                }
                return a;
            });
    ...
    async listarAdmin(req, res) {
        try {
            let agendamentos = await agendamentoService.listarAgendamentos();
            agendamentos = agendamentos.map(a => {
                const ag = a.get ? a.get({ plain: true }) : a;
                if (ag.data) {
                    const partes = ag.data.split('-');
                    if (partes.length === 3) {
                        ag.dataFormatada = `${partes[2]}/${partes[1]}/${partes[0]}`;
                    } else {
                        ag.dataFormatada = ag.data;
                    }
                }
                ag.status = { [ag.status]: true };
                return ag;
            });
    ...
    async getDetalharAgendamento(req, res) {
        try {
            let agendamento = await agendamentoService.buscarAgendamentoPorId(req.params.id);
            if (agendamento) {
                agendamento = agendamento.get ? agendamento.get({ plain: true }) : agendamento;
                if (agendamento.data) {
                    const partes = agendamento.data.split('-');
                    if (partes.length === 3) {
                        agendamento.dataFormatada = `${partes[2]}/${partes[1]}/${partes[0]}`;
                    } else {
                        agendamento.dataFormatada = agendamento.data;
                    }
                }
            }
